...
public:
    ...
    static void bucketSort(int arr[], int size) {
        const int numBuckets = 100; // Assuming 10 buckets 
        int buckets[numBuckets][MAX_SIZE] = { 0 };
        int bucketCount[numBuckets] = { 0 };

        for (int i = 0; i < size; i++) {
            int bucketIndex = (numBuckets * arr[i]) / MAX_SIZE;

            if (bucketCount[bucketIndex] < MAX_SIZE) {
                buckets[bucketIndex][bucketCount[bucketIndex]++] = arr[i];
            }
            else {
                cout << "Bucket overflow at index: " << bucketIndex << endl;
                
            }
        }

        for (int i = 0; i < numBuckets; i++) {
            sort(buckets[i], buckets[i] + bucketCount[i]);
        }
